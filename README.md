...
## Basic Information
- **Person developing model**: Jonathan Beck, jonathan.beck@gwmail.gwu.edu
...
## Training data
- **Source of training data**: https://www.kaggle.com/competitions/digit-recognizer/overview.
...
## Test Data
- **Source of test data**: https://www.kaggle.com/competitions/digit-recognizer/overview.
...
- **Potential Negative impacts of using model**:
  ...
  - **Description of unexpected results**:
    - **Data Leakage, Non-Optimal Standardization of Values, and Lengthy Training Times**
      - I'm not sure why, but the 'Tutorial' examples found on the *Overview* tab of the *Digit Recognizer* Kaggle page, illustrate and implement Feature Standardization *before* the data is split into Training and Validation subsets. Because this was my first time developing a neural network architecture, I simply made a note to self that this may serve as a potential issue. Regardless, I followed along with the general sequence of data mining techniques being presented.
      - After training the model and constructing a *Loss Curve* plot, to analyze how *Categorical Cross-Entropy* changed over increasing number of epochs, I became more suspicious that I should have trusted my gut and split the training data *before* standardization. There was a lot of variance in the resulting curve for both training and validation loss, and it hardly represented the anticipated inverted 'dimining marginal loss' pattern one would expect to see from said plot. My best model, at the time, returned a validation loss of around **0.11**.
      - I decided to analyze a confusion matrix of my results, to gauge my models *accuracy* metric and in doing so, I realized that there were severe issues with my model. The dimensions of the model didn't match between the response and input features for both the training and validation subsets, likely because of how I was naming my variables throughout preprocessing and the improper standardization techniques implemented.
      - I went back and made sure I was very careful in ensuring I effectively split the training and validation subset *before* standardization, and in doing so, my 'best model' now returned a validation loss of a little over **2.3**.
      - I decided to try adjusting some of the hyperparameters in my model (namely changing the **Patience** in *Early Stopping* to a value of 10, and lowered the number of nodes within each hidden layer, as I thought my model may have been overfitting the data). Additionally, I changed the **Standardization** technique to simply divide each value found within the data (after splitting) by the max value within the subsets, as I understand this to be a more common standardization practice for pixel related inputs, especially for simpler neural network architectures. These adjustments worked, and my new model performed admirably on both the Training and Validation data, for both **Categorical Cross Entropy** and **Accuracy**.
